...
namespace sdds
{
	...
	CustomerOrder::CustomerOrder(const std::string& coStr)
	{
		...
		////list of items
		////<algorithm>: count (InputIterator first, InputIterator last, const T& val);
		//m_cntItem = std::count(coStr.begin(), coStr.end(), ut.getDelimiter()) - 1;//...because index starts from zero, so minus 1; this line also works in a vector approach

		//if (m_cntItem > 0) {
		//	m_lstItem = new Item * [m_cntItem];
		//	for (size_t i = 0; i < m_cntItem; i++) {//assign array of pointers
		//		m_lstItem[i] = new Item(ut.extractToken(coStr, localnextpos, localmore));
		//	}
		//}
